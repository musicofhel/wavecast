#!/usr/bin/env python3
"""Experiment 5: β-NLL Heteroscedastic Loss.

Source: Seitzer et al. ICLR 2022 "On the Pitfalls of Heteroscedastic Uncertainty"
Hypothesis: Pure log-likelihood can converge to inflated variance with mediocre
predictions. β-NLL weights each sample by (σ²)^β, stabilizing variance estimation.

Pass criteria: Unconditional econ_dir ≥ baseline. Selective econ_dir > 67% at
coverage > 30% (abstain on high-variance predictions).

Usage:
    python -m scripts.feature_tests.test_beta_nll
"""
...
def run_beta_nll_test(n_seeds: int = 3) -> dict:
    """Compare CE vs β-NLL with variance-based selective trading."""
    logging.basicConfig(
        level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s"
    )
    logger.info("β-NLL experiment, %d seeds", n_seeds)
    ...
    for seed in range(n_seeds):
        ...
        for beta in betas:
            ...
            model_bnll = WaveletGPT(
                vocab_size=1, context_length=CONTEXT_LENGTH,
                task="return_quantile", n_output_classes=N_CLASSES,
                class_weights=class_weights, input_mode="continuous",
                n_aux_features=N_AUX_FEATURES,
                loss_type="beta_nll", loss_kwargs={"beta": beta},
                **MODEL_KWARGS,
            )
            t0 = time.time()
            metrics = model_bnll.fit(X_train, y_tr)
            bnll_time = time.time() - t0
            pred_bnll = model_bnll.predict(X_test).astype(np.int64)
            bnll_eval = _evaluate(
                pred_bnll, te_rets, y_test, te_valid,
                metrics["train_loss"], bnll_time,
            )

            # Get variance for selective trading
            variance = model_bnll.predict_variance(X_test)
            valid_mask = te_valid.astype(bool)
            valid_preds = pred_bnll[valid_mask]
            valid_rets = te_rets[valid_mask]
            valid_var = variance[valid_mask]

            # Selective trading: abstain on high-variance samples
            selective_results = {}
            for coverage_target in [0.3, 0.5, 0.7, 0.9]:
                ...
                if keep.sum() > 0:
                    ...
                    mid = N_CLASSES // 2
                    pred_dir = np.where(kept_preds > mid, 1, np.where(kept_preds < mid, -1, 0))
            ...
            logger.info(
                "  β=%.2f: econ=%.1f%% sharpe=%.3f var_mean=%.3f",
                beta, bnll_eval.econ_dir_accuracy * 100,
                bnll_eval.sharpe_with_costs, valid_var.mean(),
            )
    ...
    beta_summaries = {}
    for beta in betas:
        evals = [r["eval"] for r in beta_results[beta]]
        mean_econ = np.mean([e.econ_dir_accuracy for e in evals])
        mean_sharpe = np.mean([e.sharpe_with_costs for e in evals])
        mean_trans = np.mean([e.transition_accuracy for e in evals])
        ...
        beta_summaries[str(beta)] = {
            "econ_dir": float(mean_econ),
            "sharpe_costs": float(mean_sharpe),
            "transition": float(mean_trans),
            "d_econ": float(mean_econ - base_econ),
            "d_sharpe": float(mean_sharpe - base_sharpe),
            "selective": sel_summary,
        }

    # Verdict: unconditional econ ≥ baseline AND selective econ > 67% at coverage > 30%
    best_summary = beta_summaries.get(str(best_beta), {}) if best_beta else {}
    uncond_ok = best_summary.get("d_econ", -1) >= -0.01 if best_summary else False
    ...
    print()
    print("=" * w)
    print("EXPERIMENT: β-NLL Heteroscedastic Loss")
    print("=" * w)
    print(f"\n{'β':<6} {'Econ Dir':>10} {'Δ Econ':>10} {'Sharpe':>10} {'Δ Sharpe':>10} {'Trans':>10}")
    print("-" * w)
    print(f"{'base':<6} {base_econ:>9.1%} {'':>10} {base_sharpe:>+9.3f} {'':>10} {base_trans:>9.1%}")
    for beta in betas:
        s = beta_summaries[str(beta)]
        print(
            f"{beta:<6.2f} {s['econ_dir']:>9.1%} {s['d_econ']:>+9.1%} "
            f"{s['sharpe_costs']:>+9.3f} {s['d_sharpe']:>+9.3f} {s['transition']:>9.1%}"
        )
    print("-" * w)
    if best_beta is not None:
        print(f"\nBest selective: β={best_beta}, econ_dir={best_sel_econ:.1%} at coverage={best_coverage:.1%}")
    print(f"\nVERDICT: {verdict}")
    print("=" * w)
    ...
    out_path = results_dir / "beta_nll_results.json"
