#!/usr/bin/env python3
"""Validate frozen D1 model on 2026 out-of-sample data.

The model was trained on 2021-2025 and D1 was selected based on 2025 test
performance. Jan 1 – Feb 14, 2026 data has never been seen by any part of
the pipeline. This is the honest validation.

Usage:
    python scripts/validate_d1_2026.py [OPTIONS]

    --model-path PATH   Model directory (default: ~/.wavecast/models/d1_augmented_v1)
    --start DATE        Start date (default: 2026-01-01)
    --end DATE          End date (default: 2026-02-16)
    --skip-fetch        Use cached data only, don't fetch from API
"""

from __future__ import annotations

import json
import logging
import sys
import time
from dataclasses import asdict, dataclass
from pathlib import Path

import numpy as np
from numpy.typing import NDArray

from wavecast.core.types import TimeSeries
from wavecast.core.universe import DEFAULT_UNIVERSE
from wavecast.data.auxiliary_features import (
    N_AUX_FEATURES,
    compute_detail_auxiliary_features,
)
from wavecast.data.cache import ParquetCache
from wavecast.data.continuous_dataset import build_continuous_dataset
from wavecast.data.sources import fetch_massive
from wavecast.experiments.runner import SECTOR_ID_MAP
from wavecast.models.wavelet_gpt import WaveletGPT
from wavecast.targets.returns import assign_quantile_labels
from wavecast.wavelets.dwt import decompose

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger(__name__)

# --- Config ---
TICKERS = [a.ticker for a in DEFAULT_UNIVERSE.assets]
TICKER_SECTOR = {a.ticker: a.sector.value for a in DEFAULT_UNIVERSE.assets if a.sector}
SECTOR_ORDER = ["tech", "finance", "energy", "healthcare", "broad_etf", "commodity_etf"]
INTERVAL = "1h"
DETAIL_LEVELS = [1, 2, 5]
CONTEXT_LENGTH = 16
N_CLASSES = 5
MIN_RETURN_THRESHOLD = 0.001
COST_BPS = 7.0
N_RANDOM_TRIALS = 500
MIN_RELIABLE_N = 1000

# Reference metrics from Phase 9 (2025 test set)
REF_ECON_DIR = 0.600
REF_SHARPE_COSTS = 1.86
REF_TRANSITION = 0.449

DEFAULT_MODEL = Path.home() / ".wavecast" / "models" / "d1_augmented_v1"


@dataclass
class GroupResult:
    name: str
    n_test: int
    n_valid: int
    econ_dir_accuracy: float
    sharpe_raw: float
    sharpe_with_costs: float
    sharpe_percentile: float
    transition_accuracy: float
    quantile_accuracy: float
    reliable: bool


# --- Data Loading ---


def fetch_2026_data(
    start: str, end: str, rate_pause: float = 12.5
) -> dict[str, TimeSeries]:
    """Fetch 2026 hourly data for all tickers from Massive API."""
    prices: dict[str, TimeSeries] = {}
    for i, ticker in enumerate(TICKERS):
        logger.info("Fetching %s (%d/%d) %s to %s ...", ticker, i + 1, len(TICKERS), start, end)
        try:
            ts = fetch_massive(ticker, start=start, end=end, interval=INTERVAL)
            if len(ts.values) < 50:
                logger.warning("  %s: only %d bars, skipping", ticker, len(ts.values))
                continue
            prices[ticker] = ts
            logger.info("  %s: %d bars", ticker, len(ts.values))
        except Exception as e:
            logger.error("  %s: fetch failed: %s", ticker, e)
        if i < len(TICKERS) - 1:
            time.sleep(rate_pause)
    return prices


def load_cached_2026(cache: ParquetCache, start: str) -> dict[str, TimeSeries]:
    """Load 2026 data from existing cache (filter to >= start date)."""
    prices: dict[str, TimeSeries] = {}
    start_dt = np.datetime64(start)
    for ticker in TICKERS:
        ts = cache.get(ticker, INTERVAL)
        if ts is None:
            continue
        mask = ts.timestamps >= start_dt
        if mask.sum() < 50:
            continue
        prices[ticker] = TimeSeries(
            values=ts.values[mask],
            timestamps=ts.timestamps[mask],
            ticker=ticker,
            interval=INTERVAL,
        )
    return prices


def build_asset_class_map() -> dict[str, int]:
    ac_map = {}
    for a in DEFAULT_UNIVERSE.assets:
        if a.sector is not None:
            ac_map[a.ticker] = SECTOR_ID_MAP.get(a.sector.value, 0)
    return ac_map


# --- Coefficient Extractors ---


def detail_delta_coeffs(decomp, lvl):
    c = decomp.detail_at_level(lvl)
    return np.diff(c) if len(c) > 1 else c


def detail_delta_aux_fn(decomp, lvl):
    detail = decomp.detail_at_level(lvl)
    approx = decomp.approximation
    return compute_detail_auxiliary_features(detail, approx)


def build_aux_windows(aux_series, context_length, n_aux_features):
    windows = []
    for (_ticker, _level), aux in sorted(aux_series.items()):
        if len(aux) <= context_length:
            continue
        for i in range(len(aux) - context_length):
            windows.append(aux[i:i + context_length])
    if not windows:
        return np.empty((0, context_length, n_aux_features), dtype=np.float64)
    return np.array(windows, dtype=np.float64)


# --- Evaluation ---


def evaluate_group(
    name: str,
    pred_labels: NDArray,
    actual_returns: NDArray,
    actual_labels: NDArray,
    valid_mask: NDArray,
) -> GroupResult:
    """Full evaluation on a group."""
    n_test = len(pred_labels)
    n_valid = int(valid_mask.sum())
    mid = N_CLASSES // 2

    pred_dir = np.zeros(n_test, dtype=np.float64)
    pred_dir[pred_labels > mid] = 1.0
    pred_dir[pred_labels < mid] = -1.0
    actual_dir = np.sign(actual_returns)

    # Economic directional accuracy (filtered)
    filt = valid_mask & (np.abs(actual_returns) > MIN_RETURN_THRESHOLD)
    if filt.sum() > 0:
        has_pred = pred_dir[filt] != 0
        if has_pred.sum() > 0:
            econ_dir_acc = float(np.mean(pred_dir[filt][has_pred] == actual_dir[filt][has_pred]))
        else:
            econ_dir_acc = 0.5
    else:
        econ_dir_acc = 0.5

    # Quantile accuracy
    q_acc = float(np.mean(pred_labels[valid_mask] == actual_labels[valid_mask])) if n_valid > 0 else 0.0

    # Sharpe
    if n_valid > 0:
        pnl_raw = pred_dir[valid_mask] * actual_returns[valid_mask]
        pnl_raw = pnl_raw[~np.isnan(pnl_raw)]
        dir_changes = np.abs(np.diff(pred_dir[valid_mask]))
        cost_per_step = np.zeros(n_valid)
        cost_per_step[1:] = dir_changes * (COST_BPS / 10000)
        cost_per_step[0] = abs(pred_dir[valid_mask][0]) * (COST_BPS / 10000)
        pnl_net = pred_dir[valid_mask] * actual_returns[valid_mask] - cost_per_step
        pnl_net = pnl_net[~np.isnan(pnl_net)]

        sharpe_raw = (
            float(np.mean(pnl_raw) / np.std(pnl_raw) * np.sqrt(252 * 7))
            if len(pnl_raw) > 1 and np.std(pnl_raw) > 0 else 0.0
        )
        sharpe_costs = (
            float(np.mean(pnl_net) / np.std(pnl_net) * np.sqrt(252 * 7))
            if len(pnl_net) > 1 and np.std(pnl_net) > 0 else 0.0
        )
    else:
        sharpe_raw = 0.0
        sharpe_costs = 0.0

    # Sharpe vs random
    rng = np.random.default_rng(42)
    random_sharpes = []
    if n_valid > 0:
        valid_rets = actual_returns[valid_mask]
        valid_rets_clean = valid_rets[~np.isnan(valid_rets)]
        for _ in range(N_RANDOM_TRIALS):
            rand_dir = rng.choice([-1.0, 0.0, 1.0], size=len(valid_rets_clean))
            rand_pnl = rand_dir * valid_rets_clean
            if len(rand_pnl) > 1 and np.std(rand_pnl) > 0:
                random_sharpes.append(
                    float(np.mean(rand_pnl) / np.std(rand_pnl) * np.sqrt(252 * 7))
                )
    sharpe_pctile = (
        float(np.mean(np.array(random_sharpes) < sharpe_costs) * 100)
        if random_sharpes else 50.0
    )

    # Transition accuracy
    if n_valid > 10:
        valid_idx = np.where(valid_mask)[0]
        actual_dir_valid = actual_dir[valid_idx]
        pred_dir_valid = pred_dir[valid_idx]
        transitions = np.where(np.diff(np.sign(actual_dir_valid)) != 0)[0] + 1
        if len(transitions) > 0:
            trans_acc = float(np.mean(pred_dir_valid[transitions] == actual_dir_valid[transitions]))
        else:
            trans_acc = 0.5
    else:
        trans_acc = 0.5

    return GroupResult(
        name=name,
        n_test=n_test,
        n_valid=n_valid,
        econ_dir_accuracy=econ_dir_acc,
        sharpe_raw=sharpe_raw,
        sharpe_with_costs=sharpe_costs,
        sharpe_percentile=sharpe_pctile,
        transition_accuracy=trans_acc,
        quantile_accuracy=q_acc,
        reliable=n_valid >= MIN_RELIABLE_N,
    )


def verdict(r: GroupResult) -> str:
    if not r.reliable:
        return "LOW-N"
    if r.econ_dir_accuracy > 0.55 and r.sharpe_with_costs > 0.5:
        return "PASS"
    if r.econ_dir_accuracy > 0.52:
        return "MARG"
    if r.econ_dir_accuracy >= 0.48:
        return "NOISE"
    return "HARMFUL"


# --- Main ---


def main() -> None:
    model_path = DEFAULT_MODEL
    start_date = "2026-01-01"
    end_date = "2026-02-16"
    skip_fetch = False

    for arg in sys.argv[1:]:
        if arg.startswith("--model-path="):
            model_path = Path(arg.split("=", 1)[1])
        elif arg.startswith("--start="):
            start_date = arg.split("=", 1)[1]
        elif arg.startswith("--end="):
            end_date = arg.split("=", 1)[1]
        elif arg == "--skip-fetch":
            skip_fetch = True

    # Verify model exists
    model_dir = Path(model_path)
    if not (model_dir / "config.json").exists():
        logger.error("Model not found at %s — run scripts/train_d1_model.py first", model_path)
        sys.exit(1)
    boundaries_path = model_dir / "quantile_boundaries.json"
    if not boundaries_path.exists():
        logger.error("Quantile boundaries not found at %s", boundaries_path)
        sys.exit(1)

    # Load model
    logger.info("Loading frozen model from %s", model_path)
    model = WaveletGPT.load(model_dir)

    # Load quantile boundaries
    with open(boundaries_path) as f:
        boundaries_raw = json.load(f)
    boundaries: dict[int, NDArray] = {
        int(k): np.array(v, dtype=np.float64) for k, v in boundaries_raw.items()
    }
    logger.info("Loaded boundaries for levels: %s", list(boundaries.keys()))

    # Fetch 2026 data
    if skip_fetch:
        cache = ParquetCache(Path.home() / ".wavecast" / "cache")
        test_prices = load_cached_2026(cache, start_date)
    else:
        test_prices = fetch_2026_data(start_date, end_date)

    if not test_prices:
        logger.error("No 2026 data available")
        sys.exit(1)

    tickers = sorted(test_prices.keys())
    logger.info("Loaded %d tickers for 2026 validation", len(tickers))
    for t in tickers:
        ts = test_prices[t]
        logger.info("  %s: %d bars (%s to %s)", t, len(ts.values),
                     ts.timestamps[0], ts.timestamps[-1])

    ac_map = build_asset_class_map()

    # --- Build D1 datasets ---
    test_series: dict[tuple[str, int], NDArray] = {}
    test_aux_series: dict[tuple[str, int], NDArray] = {}

    for ticker in tickers:
        test_decomp = decompose(test_prices[ticker], level=5)
        for lvl in DETAIL_LEVELS:
            ec = detail_delta_coeffs(test_decomp, lvl)
            if len(ec) > CONTEXT_LENGTH:
                test_series[(ticker, lvl)] = ec
                test_aux_series[(ticker, lvl)] = detail_delta_aux_fn(test_decomp, lvl)

    test_ds = build_continuous_dataset(test_series, CONTEXT_LENGTH, ac_map, normalize=True)
    logger.info("Test dataset: %d windows", len(test_ds.windows))

    if len(test_ds.windows) == 0:
        logger.error("No windows built from 2026 data")
        sys.exit(1)

    # --- Compute returns ---
    test_rets = np.full(len(test_ds.windows), np.nan)
    test_valid = np.zeros(len(test_ds.windows), dtype=np.bool_)
    for i, w in enumerate(test_ds.windows):
        key = (w.ticker, w.level)
        if key not in test_series or w.ticker not in test_prices:
            continue
        coeffs_len = len(test_series[key])
        target_pos = w.token_position
        if target_pos >= coeffs_len:
            continue
        span = 2 ** w.level
        bar_start = target_pos * span
        bar_end = bar_start + span
        pv = test_prices[w.ticker].values
        if bar_end < len(pv) and pv[bar_start] > 0:
            test_rets[i] = (pv[bar_end] - pv[bar_start]) / pv[bar_start]
            test_valid[i] = True

    test_lvls = np.array([w.level for w in test_ds.windows], dtype=np.int64)
    logger.info("Valid returns: %d / %d (%.1f%%)",
                test_valid.sum(), len(test_ds.windows),
                100 * test_valid.sum() / len(test_ds.windows))

    # --- Assign ground truth labels using SAVED boundaries ---
    y_test = assign_quantile_labels(test_rets, test_lvls, boundaries)

    # --- Build X array ---
    ctx_arr, lvl_arr, ac_arr = test_ds.to_arrays()
    test_aux_win = build_aux_windows(test_aux_series, CONTEXT_LENGTH, N_AUX_FEATURES)
    test_aux_flat = test_aux_win.reshape(len(test_aux_win), -1)
    X_test = np.column_stack([ctx_arr, test_aux_flat, lvl_arr, ac_arr])

    # --- Predict with frozen model ---
    logger.info("Running predictions on %d windows...", len(X_test))
    predicted = model.predict(X_test).astype(np.int64)

    # --- Build per-sample metadata ---
    test_tickers = np.array([w.ticker for w in test_ds.windows])
    test_levels = np.array([w.level for w in test_ds.windows], dtype=np.int64)
    test_sectors = np.array([TICKER_SECTOR.get(w.ticker, "unknown") for w in test_ds.windows])

    # ===== AGGREGATE =====
    agg = evaluate_group("AGGREGATE", predicted, test_rets, y_test, test_valid)

    # ===== PER-LEVEL =====
    level_results: list[GroupResult] = []
    for lvl in sorted(DETAIL_LEVELS):
        mask = test_levels == lvl
        idx = np.where(mask)[0]
        r = evaluate_group(f"level_{lvl}", predicted[idx], test_rets[idx], y_test[idx], test_valid[idx])
        level_results.append(r)

    # ===== PER-TICKER =====
    ticker_results: list[GroupResult] = []
    for ticker in sorted(set(test_tickers)):
        mask = test_tickers == ticker
        idx = np.where(mask)[0]
        r = evaluate_group(ticker, predicted[idx], test_rets[idx], y_test[idx], test_valid[idx])
        ticker_results.append(r)

    # ===== PER-SECTOR =====
    sector_results: list[GroupResult] = []
    for sector in SECTOR_ORDER:
        mask = test_sectors == sector
        if not mask.any():
            continue
        idx = np.where(mask)[0]
        r = evaluate_group(sector, predicted[idx], test_rets[idx], y_test[idx], test_valid[idx])
        sector_results.append(r)

    # ===== LEVEL-1-ONLY (same model, filtered test set) =====
    l1_mask = test_levels == 1
    l1_idx = np.where(l1_mask)[0]
    l1_result = evaluate_group(
        "LEVEL-1-ONLY", predicted[l1_idx], test_rets[l1_idx], y_test[l1_idx], test_valid[l1_idx]
    )

    # ===== Print Results =====
    w = 120
    print()
    print("=" * w)
    print("D1 VALIDATION ON 2026 OUT-OF-SAMPLE DATA")
    print(f"Model: {model_path} (trained 2021-2025)")
    print(f"Test period: {start_date} to {end_date}")
    print(f"Tickers: {len(tickers)}/{len(TICKERS)}")
    print("=" * w)

    print("\nAGGREGATE RESULTS")
    print(f"  Samples:          {agg.n_test}")
    print(f"  Valid:            {agg.n_valid}")
    print(f"  Econ Dir Acc:     {agg.econ_dir_accuracy:.1%}     (2025 test: {REF_ECON_DIR:.1%})")
    print(f"  Sharpe (raw):     {agg.sharpe_raw:+.3f}")
    print(f"  Sharpe (+costs):  {agg.sharpe_with_costs:+.3f}    (2025 test: {REF_SHARPE_COSTS:+.3f})")
    print(f"  vs Random:        {agg.sharpe_percentile:.1f}%")
    print(f"  Transition Acc:   {agg.transition_accuracy:.1%}     (2025 test: {REF_TRANSITION:.1%})")
    print(f"  Quantile Acc:     {agg.quantile_accuracy:.1%}")

    degradation = REF_ECON_DIR - agg.econ_dir_accuracy
    print(f"\n  2025 -> 2026 degradation: {degradation:+.1%}")

    # Per-level
    print("\n" + "=" * w)
    print("PER-LEVEL BREAKDOWN")
    print("=" * w)
    print(f"{'Level':<10} {'Samples':>8} {'Valid':>7} {'Econ Dir':>8} {'S+Cost':>7} "
          f"{'vs Rnd':>7} {'Trans':>6} {'Q.Acc':>6}  Verdict")
    print("-" * w)
    for r in level_results:
        v = verdict(r)
        rel = "" if r.reliable else " *"
        print(f"{r.name:<10} {r.n_test:>8d} {r.n_valid:>7d} {r.econ_dir_accuracy:>7.1%} "
              f"{r.sharpe_with_costs:>+7.3f} {r.sharpe_percentile:>6.1f}% "
              f"{r.transition_accuracy:>5.1%} {r.quantile_accuracy:>5.1%}  {v}{rel}")

    # Per-ticker
    print("\n" + "=" * w)
    print("PER-TICKER BREAKDOWN")
    print("=" * w)
    print(f"{'Ticker':<7} {'Sector':<14} {'Valid':>7} {'Econ Dir':>8} "
          f"{'S+Cost':>7} {'vs Rnd':>7} {'Trans':>6} {'Q.Acc':>6}  Verdict")
    print("-" * w)
    for r in sorted(ticker_results, key=lambda x: x.econ_dir_accuracy, reverse=True):
        v = verdict(r)
        rel = "" if r.reliable else " *"
        print(f"{r.name:<7} {TICKER_SECTOR.get(r.name, '?'):<14} {r.n_valid:>7d} "
              f"{r.econ_dir_accuracy:>7.1%} {r.sharpe_with_costs:>+7.3f} "
              f"{r.sharpe_percentile:>6.1f}% {r.transition_accuracy:>5.1%} "
              f"{r.quantile_accuracy:>5.1%}  {v}{rel}")
    print("-" * w)
    print(f"{'AGG':<7} {'all':<14} {agg.n_valid:>7d} "
          f"{agg.econ_dir_accuracy:>7.1%} {agg.sharpe_with_costs:>+7.3f} "
          f"{agg.sharpe_percentile:>6.1f}% {agg.transition_accuracy:>5.1%} "
          f"{agg.quantile_accuracy:>5.1%}  {verdict(agg)}")

    # Per-sector
    print("\n" + "=" * w)
    print("PER-SECTOR BREAKDOWN")
    print("=" * w)
    print(f"{'Sector':<14} {'Valid':>7} {'Econ Dir':>8} {'S+Cost':>7} "
          f"{'vs Rnd':>7} {'Trans':>6} {'Q.Acc':>6}  Verdict")
    print("-" * w)
    for r in sector_results:
        v = verdict(r)
        rel = "" if r.reliable else " *"
        print(f"{r.name:<14} {r.n_valid:>7d} {r.econ_dir_accuracy:>7.1%} "
              f"{r.sharpe_with_costs:>+7.3f} {r.sharpe_percentile:>6.1f}% "
              f"{r.transition_accuracy:>5.1%} {r.quantile_accuracy:>5.1%}  {v}{rel}")

    # Level-1-only
    print("\n" + "=" * w)
    print("LEVEL-1-ONLY RESULTS (same model, filtered test set)")
    print("=" * w)
    print(f"  Samples:          {l1_result.n_test} (vs {agg.n_test} full)")
    print(f"  Valid:            {l1_result.n_valid}")
    print(f"  Econ Dir Acc:     {l1_result.econ_dir_accuracy:.1%}")
    print(f"  Sharpe (+costs):  {l1_result.sharpe_with_costs:+.3f}")
    print(f"  vs Random:        {l1_result.sharpe_percentile:.1f}%")
    print(f"  Transition Acc:   {l1_result.transition_accuracy:.1%}")
    print(f"  Quantile Acc:     {l1_result.quantile_accuracy:.1%}")

    # Verdict
    print("\n" + "=" * w)
    if agg.econ_dir_accuracy > 0.58:
        overall_verdict = "PASS (STRONG)"
        interpretation = "Edge is real and robust"
    elif agg.econ_dir_accuracy > 0.55:
        overall_verdict = "PASS"
        interpretation = "Edge exists, proceed with feature tests and forward testing"
    elif agg.econ_dir_accuracy > 0.52:
        overall_verdict = "MARGINAL"
        interpretation = "Marginal edge, may not survive costs at scale"
    else:
        overall_verdict = "FAIL"
        interpretation = "D1 was overfit to 2025 — back to representation level"

    print(f"OVERALL VERDICT: {overall_verdict}")
    print(f"  {interpretation}")
    print(f"  Aggregate econ dir: {agg.econ_dir_accuracy:.1%} (2025: {REF_ECON_DIR:.1%}, degradation: {degradation:+.1%})")
    print(f"  Level-1-only:       {l1_result.econ_dir_accuracy:.1%}")

    passing = [r for r in ticker_results if verdict(r) == "PASS"]
    marginal = [r for r in ticker_results if verdict(r) == "MARG"]
    noise = [r for r in ticker_results if verdict(r) in ("NOISE", "HARMFUL")]
    low_n = [r for r in ticker_results if verdict(r) == "LOW-N"]
    print(f"\n  Tickers PASS: {len(passing)}/{len(ticker_results)}")
    if passing:
        print(f"    {', '.join(r.name for r in passing)}")
    if marginal:
        print(f"  Tickers MARG: {len(marginal)} — {', '.join(r.name for r in marginal)}")
    if noise:
        print(f"  Tickers NOISE/HARM: {len(noise)} — {', '.join(r.name for r in noise)}")
    if low_n:
        print(f"  Tickers LOW-N: {len(low_n)} — {', '.join(r.name for r in low_n)}")

    print("=" * w)

    # --- Save JSON ---
    results_dir = Path.home() / ".wavecast" / "audit"
    results_dir.mkdir(parents=True, exist_ok=True)
    output = {
        "test_period": {"start": start_date, "end": end_date},
        "model_path": str(model_path),
        "n_tickers": len(tickers),
        "tickers_loaded": tickers,
        "aggregate": asdict(agg),
        "level_1_only": asdict(l1_result),
        "per_level": [asdict(r) for r in level_results],
        "per_ticker": [asdict(r) for r in ticker_results],
        "per_sector": [asdict(r) for r in sector_results],
        "verdicts": {r.name: verdict(r) for r in ticker_results},
        "overall_verdict": overall_verdict,
        "degradation_pp": round(degradation * 100, 1),
        "reference_2025": {
            "econ_dir": REF_ECON_DIR,
            "sharpe_costs": REF_SHARPE_COSTS,
            "transition": REF_TRANSITION,
        },
    }
    out_path = results_dir / "d1_2026_validation.json"
    with open(out_path, "w") as f:
        json.dump(output, f, indent=2)
    logger.info("Results saved to %s", out_path)


if __name__ == "__main__":
    main()
